        ...
        <div className="flex-1 relative flex overflow-hidden">
          <div className={`flex-1 flex transition-all duration-1000 ${activeTab === 'MEMORY' || activeTab === 'SETTINGS' ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
            ...
            <main className="flex-1 relative flex flex-col items-center justify-center px-6">
                <div className="relative z-10"><canvas ref={canvasRef} width={800} height={800} className="w-[600px] h-[600px]" /></div>
          ...
          <AnimatePresence>
            {activeTab === "SETTINGS" && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="absolute inset-0 bg-[#010409] z-[60] flex flex-col items-center p-24 overflow-y-auto">
                <div className="w-full max-w-3xl space-y-20 pb-24">
                  <section className="bg-[#0d1117] p-10 rounded-[40px] border border-white/[0.05] flex items-center justify-between">
                    <div className="flex items-center gap-8">
                      ...
                      <div><h2 className="text-white text-xl font-light tracking-tight">{user?.fullName}</h2><p className="text-[10px] text-slate-500 font-mono mt-1">{user?.primaryEmailAddress?.emailAddress}</p></div>
                    </div>
                  </section>
